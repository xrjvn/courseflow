    ...
    <div className="flex flex-1 flex-col min-h-0">
      ...
      <div className="lg:flex lg:flex-auto lg:flex-col min-h-0">
        ...
        <div className="flex text-xs leading-6 lg:flex-auto min-h-0">
          <div className="hidden w-full border-x border-border-subtle lg:grid lg:grid-cols-7 lg:grid-rows-5 overflow-hidden">
            {days.map((day, dayIdx) => (
          ...
          <div className="isolate grid w-full grid-cols-7 grid-rows-5 border-x border-border-subtle lg:hidden overflow-hidden">
            {days.map((day, dayIdx) => (
              <FullScreenCalendarDay
                key={dayIdx}
                data={data}
  ...
  if (!isDesktop) {
    return (
      <button
        ...
        "flex h-14 flex-col border-b border-r border-border-subtle px-3 py-2 hover:bg-bg-elevated focus:z-10"
        )}
      >
        <time
          dateTime={dateKey}
        ...
        {dayEvents.length > 0 && (
          <div className="mt-1.5 flex w-full flex-col gap-1">
            {dayEvents.slice(0, 1).map((event) => (
              <div key={event.id} className={eventPillClasses(event.name)}>
  ...
  return (
    <div
      ...
      className={cn(
        ...
        "relative flex flex-col border-b border-r border-border-subtle hover:bg-bg-elevated focus:z-10",
        !isEqual(day, selectedDay) && "hover:bg-[rgba(255,255,255,0.04)]"
      )}
    >
